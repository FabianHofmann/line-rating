...
if __name__ == "__main__":
    ...
    curtailment = pd.DataFrame(
        {name: get_curtailment(n) for name, n in networks.items()}
    ).T
    ...
    fig, ax = plt.subplots()
    capacity.div(1e3).plot.area(
        ax=ax, color=ccolors[capacity.columns], legend="reverse"
    )
    ax.legend(title="", bbox_to_anchor=(1, 1), loc="upper left")
    ax.set_xlabel("Maximally allowed rating")
    ...
    fig, ax = plt.subplots()
    costs.loc[1.0, :].sub(costs).sum(1).div(1e6).plot(
        ax=ax, color=gcolors[costs.columns]
    )
    ax.set_xlabel("Maximally allowed rating")
