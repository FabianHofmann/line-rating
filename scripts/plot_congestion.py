...
if __name__ == "__main__":
    ...
    for key in n.keys():
        if key =="nolr":
            line_util.update({key: np.abs(n[key].lines_t.p0).divide(n[key].lines.s_nom_opt*n[key].lines.s_max_pu, axis=1)})
        elif key=="lr":
    ...
    fig.suptitle('Congestion comparison between power system with static and dynamic line rating', fontsize=20)
    ...
    for i, figure in enumerate(figures):
        ...
        for line in plot_data_lines.index:
            ...
            ax[i].plot([plot_data_lines.loc[line]["x0"],plot_data_lines.loc[line]["x1"]], [plot_data_lines.loc[line]["y0"],plot_data_lines.loc[line]["y1"]], color=cm.YlOrBr(line_util[f"{figure}_mean_nom"][line]), linewidth=0.5+line_util[f"{figure}_mean_nom"][line]*2)
