...
if __name__ == "__main__":
    ...
    ###Generator Expansion Data

    results = ["nolr", "lr"]
    gen_expansion = {}
    for result in results:
        gen_expansion.update(
            {
                result: n[result]
                .generators.groupby(["bus", "carrier"])
                .sum()["p_nom_opt"]
                - n[result].generators.groupby(["bus", "carrier"]).sum()["p_nom"]
            }
        )
    ...
    max_wind_expansion = np.max(
        [
            gen_expansion[result][
                np.core.defchararray.find(
                    gen_expansion[result].index.get_level_values(1).values.astype(str),
                    "wind",
                )
                != -1
            ]
    ...
    fig.suptitle(
        "Wind expansion comparison between power system with static and dynamic line rating",
        fontsize=20,
    )
    fig.subplots_adjust(
    ...
    for i, figure in enumerate(figures):
        ...
        for j, bus in enumerate(plot_data_buses.index):
            ...
            subax[j].bar(
                ...
                alpha=0.95,
                color="aqua",
                height=gen_expansion[figure][
                    np.core.defchararray.find(
                        gen_expansion[figure]
