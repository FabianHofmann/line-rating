...
# TODO: merge this with into plot_grid_stats and rewrite with seaborn to make colorbar below plot
...
def plot_congestion_correlation(networks):
    ...
    """
    fig, axes = plt.subplots(1, 2, figsize=(4.5, 5), sharey=True)
    cmap = plt.get_cmap("viridis")
    max_curtailment = list()
    for (n, ax) in zip(networks, axes):
        curtailment = (
            (n.generators_t.p_max_pu * n.generators.p_nom_opt).subtract(
                n.generators_t.p, axis="columns"
            )
        ).sum(axis=1)
        max_curtailment.append(curtailment.max())
        congestion = (
            n.pnl("Line").mu_lower.abs() + n.pnl("Line").mu_upper.abs()
        ).round(3) != 0
        ...
        over_capacity = (n.generators_t.p_max_pu * n.generators.p_nom_opt).sum(
            axis=1
        ) / n.loads_t.p_set.sum(axis=1)
        sc = ax.scatter(
            x=over_capacity,
            y=cogestion,
            s=curtailment_norm * 100,
            c=curtailment,
            cmap=cmap,
            zorder=5,
        )
    ...
    xlim = max(ax.get_xlim() for ax in axes)
    for ax in axes:
        ax.set_xlim(0, xlim[1] * 1.1)
    ...
    fig.colorbar(
        sm, orientation="vertical", format=formatter_func, label="Curtailment in GWh"
    )
    return fig
...
def plot_wind_congestion_correlation(networks):
    """
    This function plots the relation between congestion , wind overcapacity
    (renewable potential/load) and wind curtailment.

    This figure helps to understand how much wind is curtailed due to
    congestion and how much due to over supply.
    """
    ...
    for (n, ax) in zip(networks, axes):
        ...
        curtailment = (
            (
                n.generators_t.p_max_pu[filter_wind]
                * n.generators.p_nom_opt.loc[filter_wind]
            ).subtract(n.generators_t.p[filter_wind], axis="columns")
        ).sum(axis=1)
        max_curtailment.append(curtailment.max())
        congestion = (
            n.pnl("Line").mu_lower.abs() + n.pnl("Line").mu_upper.abs()
        ...
        over_capacity = (
            n.generators_t.p_max_pu[filter_wind]
            * n.generators.p_nom_opt.loc[filter_wind]
        ).sum(axis=1) / n.loads_t.p_set.sum(axis=1)
        sc = ax.scatter(
            x=over_capacity,
            y=cogestion,
            s=curtailment_norm * 100,
        ...
        ax.yaxis.set_tick_params(which="both")
    for ax in axes:
        ax.set_xlim(max(ax.get_xlim() for ax in axes))
    axes[0].set_ylabel("Number of Lines Congested")
    norm = mpl.colors.Normalize(vmin=0, vmax=max(max_curtailment))
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
